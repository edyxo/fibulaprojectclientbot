...
void Creature::stopMove(bool checkNextMovement)
{
	Sint32 smoothOffsetX = 0;
	Sint32 smoothOffsetY = 0;
	if(m_isLocalCreature && m_walking)
	{
		smoothOffsetX = getOffsetX(true);
		smoothOffsetY = getOffsetY(true);
	}
	...
	if(m_isLocalCreature)
	{
		startWalkSmoothing(smoothOffsetX, smoothOffsetY);
		if(checkNextMovement)
		{
			if(g_engine.shouldPauseWalkForManualAction())
				g_engine.markManualActionPausedWalk();
			else
...
void Creature::updateDrawnTile()
{
	...
	std::pair<Sint32, Sint32> displacement = getDisplacement();
	Sint32 virtualRight = getRawOffsetX() - displacement.first + 31;
	Sint32 virtualBottom = getRawOffsetY() - displacement.second + 31;

	Sint32 tileOffsetX = (virtualRight >= 0 ? virtualRight / 32 : -(((-virtualRight) + 31) / 32));
	Sint32 tileOffsetY = (virtualBottom >= 0 ? virtualBottom / 32 : -(((-virtualBottom) + 31) / 32));
	tileOffsetX = UTIL_max<Sint32>(-1, UTIL_min<Sint32>(1, tileOffsetX));
	tileOffsetY = UTIL_max<Sint32>(-1, UTIL_min<Sint32>(1, tileOffsetY));

	Sint32 drawX = SDL_static_cast(Sint32, m_walkToPosition.x) + tileOffsetX;
	Sint32 drawY = SDL_static_cast(Sint32, m_walkToPosition.y) + tileOffsetY;
	if(drawX < 0 || drawY < 0)
		return;

	Position drawPosition(SDL_static_cast(Uint16, drawX), SDL_static_cast(Uint16, drawY), m_walkToPosition.z);
	Tile* tile = g_map.getTile(drawPosition);
	if(!tile)
		return;

	m_walkOffsetX = tileOffsetX * 32;
	m_walkOffsetY = tileOffsetY * 32;
	if(tile != m_drawnTile)
		addToDrawnTile(tile);
}
...
void Creature::update()
{
	if(m_updateTime != g_frameTime)
	{
		...
		bool smoothingAnimation = (m_walkSmoothingAnimating && m_walkSmoothingStartTime != 0 && m_walkSmoothingDuration != 0);
		if(m_walking)
		{
		...
		else if(smoothingAnimation)
		{
			Uint32 elapsed = g_frameTime - m_walkSmoothingStartTime;
			if(elapsed >= m_walkSmoothingDuration)
			{
				clearWalkSmoothing();
				smoothingAnimation = false;
				m_walkedPixels = 0;
				m_currentFrame = ThingFrameGroup_Idle;
			}
			else
			{
				m_walkDirection = m_walkSmoothingDirection;
				m_currentFrame = (!g_game.hasGameFeature(GAME_FEATURE_FRAMEGROUPS) || (m_thingType && m_thingType->hasFlag(ThingAttribute_AnimateAlways)) ? ThingFrameGroup_Idle : ThingFrameGroup_Moving);
				m_walkedPixels = UTIL_min<Sint32>(31, SDL_static_cast(Sint32, (SDL_static_cast(Uint64, elapsed) * 32) / m_walkSmoothingDuration));
			}
		}
		...
		if(m_thingType)
		{
			...
			else
			{
				...
				else if(m_walking || smoothingAnimation)
				{
					Uint8 animCount = m_thingType->m_frameGroup[m_currentFrame].m_animCount;
...
void Creature::renderInformations(Sint32 posX, Sint32 posY, Sint32 drawX, Sint32 drawY, float scale, bool visible)
{
	...
	{
		char hpBuf[8];
		SDL_snprintf(hpBuf, sizeof(hpBuf), "%d%%", SDL_static_cast(Sint32, m_health));
		g_engine.drawFont(CLIENT_FONT_OUTLINED, posX, posY + 12, std::string(hpBuf), 255, 255, 255, CLIENT_FONT_ALIGN_CENTER);
	}
	if(g_engine.isExhausted(m_id))
	{
		float exhaustRatio = g_engine.getExhaustRatio(m_id);
		renderer->fillRectangle(posX - 14, posY + 17, 28, 3, 0, 0, 0, 180);
		renderer->fillRectangle(posX - 13, posY + 18, SDL_static_cast(Sint32, exhaustRatio * 26.0f), 1, 255, 140, 0, 220);
	}
...
Sint32 Creature::getRawOffsetX(bool checkPreWalk)
{
...
Sint32 Creature::getRawOffsetY(bool checkPreWalk)
{
	if(!m_walking) return 0;
	switch(m_walkDirection)
	{
...
Sint32 Creature::getWalkSmoothingOffsetX()
{
	if(m_walkSmoothingStartTime == 0)
		return 0;
	...
	Uint32 remaining = m_walkSmoothingDuration - elapsed;
	return SDL_static_cast(Sint32, (SDL_static_cast(Sint64, m_walkSmoothingOffsetX) * remaining) / m_walkSmoothingDuration);
}

Sint32 Creature::getWalkSmoothingOffsetY()
{
	if(m_walkSmoothingStartTime == 0)
		return 0;
	...
	Uint32 remaining = m_walkSmoothingDuration - elapsed;
	return SDL_static_cast(Sint32, (SDL_static_cast(Sint64, m_walkSmoothingOffsetY) * remaining) / m_walkSmoothingDuration);
}

void Creature::startWalkSmoothing(Sint32 offsetX, Sint32 offsetY)
{
	if(!m_isLocalCreature)
		return;

	Sint32 absX = (offsetX < 0 ? -offsetX : offsetX);
	Sint32 absY = (offsetY < 0 ? -offsetY : offsetY);
	Sint32 largestOffset = UTIL_max<Sint32>(absX, absY);
	if(largestOffset <= 1)
	{
		clearWalkSmoothing();
		return;
	...
	// Server corrections can otherwise snap the camera/player by up to a full
	// tile. Keep the authoritative position, but fade the visual offset briefly.
	m_walkSmoothingOffsetX = offsetX;
	m_walkSmoothingOffsetY = offsetY;
	m_walkSmoothingDuration = UTIL_min<Uint32>(140, UTIL_max<Uint32>(70, SDL_static_cast(Uint32, largestOffset * 4)));
	m_walkSmoothingStartTime = g_frameTime;
	m_walkSmoothingDirection = m_walkDirection;
	m_walkSmoothingAnimating = true;
	m_walkedPixels = 0;
}

void Creature::clearWalkSmoothing()
{
	m_walkSmoothingStartTime = 0;
	m_walkSmoothingDuration = 0;
	m_walkSmoothingOffsetX = 0;
	m_walkSmoothingOffsetY = 0;
	m_walkSmoothingAnimating = false;
}

Sint32 Creature::getOffsetX(bool checkPreWalk)
{
	Sint32 offset = getRawOffsetX(checkPreWalk);
	if(m_isLocalCreature)
		offset += getWalkSmoothingOffsetX();
	return offset;
}

Sint32 Creature::getOffsetY(bool checkPreWalk)
{
	Sint32 offset = getRawOffsetY(checkPreWalk);
	if(m_isLocalCreature)
		offset += getWalkSmoothingOffsetY();
	return offset;
}

...
void Chat::gameStart()
{
	...
	m_textbox->setText("");
	m_chatInputActive = !g_engine.isWASDMode();
}
...
void Chat::onKeyDown(SDL_Event& event)
{
	...
	if((event.key.keysym.mod & KMOD_CTRL) && event.key.keysym.sym == SDLK_c &&
	   selectedchannel->channelConsole && selectedchannel->channelConsole->hasSelection())
	{
		SDL_Event copyEvent = event;
		copyEvent.key.keysym.mod = KMOD_CTRL;
		selectedchannel->channelConsole->onKeyDown(copyEvent);
		return;
	}

	SDL_Event textEvent = event;
	if(event.key.keysym.mod & KMOD_CTRL)
		textEvent.key.keysym.mod = KMOD_CTRL;
	m_textbox->onKeyDown(textEvent);
}
...
void Chat::onLMouseDown(iRect& rect, Sint32 x, Sint32 y)
{
	...
	if(!g_engine.getHideWASD())
	{
		iRect wasdRect = iRect(rect.x1 + rect.x2 - 36, rect.y1 + rect.y2 - 22, 36, 18);
		if(wasdRect.isPointInside(x, y))
		{
			g_engine.setWASDMode(!g_engine.isWASDMode());
			return;
		}
	}
...
void Chat::render(iRect& rect)
{
	...
	if(!g_engine.getHideWASD())
	{
		...
		if(g_engine.isWASDMode())
		{
			if(m_chatInputActive) { wr = 160; wg = 160; wb = 160; }
			else                  { wr = 96;  wg = 200; wb = 96;  }
		}
		else { wr = 60; wg = 60; wb = 60; }
		g_engine.drawFont(CLIENT_FONT_OUTLINED, rect.x1 + rect.x2 - 34, rect.y1 + rect.y2 - 19, "WASD", wr, wg, wb, CLIENT_FONT_ALIGN_LEFT);
	}

	if(!g_engine.isWASDMode() || m_chatInputActive)
	{
		if(!m_textbox->isActive())
			m_textbox->activate();
		iRect tRect = iRect(rect.x1 + 23, rect.y1 + rect.y2 - 20, rect.x2 - 61, 16);

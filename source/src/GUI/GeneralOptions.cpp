...
#define GENERAL_OPTIONS_HEIGHT 576
...
#define GENERAL_OPTIONS_EXTRA_RIGHT_TEXT "Extra Right Sidebar"
#define GENERAL_OPTIONS_EXTRA_RIGHT_X 18
#define GENERAL_OPTIONS_EXTRA_RIGHT_Y 296
#define GENERAL_OPTIONS_EXTRA_RIGHT_W 210
#define GENERAL_OPTIONS_EXTRA_RIGHT_H 22
#define GENERAL_OPTIONS_EXTRA_RIGHT_EVENTID 1014
#define GENERAL_OPTIONS_EXTRA_LEFT_TEXT "Extra Left Sidebar"
#define GENERAL_OPTIONS_EXTRA_LEFT_X 18
#define GENERAL_OPTIONS_EXTRA_LEFT_Y 329
#define GENERAL_OPTIONS_EXTRA_LEFT_W 210
#define GENERAL_OPTIONS_EXTRA_LEFT_H 22
#define GENERAL_OPTIONS_EXTRA_LEFT_EVENTID 1015
#define GENERAL_OPTIONS_SHOW_ACTION1_TEXT "Show Action Bar 1"
#define GENERAL_OPTIONS_SHOW_ACTION1_X 18
#define GENERAL_OPTIONS_SHOW_ACTION1_Y 362
...
#define GENERAL_OPTIONS_SHOW_ACTION2_Y 395
...
#define GENERAL_OPTIONS_ASK_BUY_PRODUCTS_Y 428
...
#define GENERAL_OPTIONS_DEV_MODE_TEXT "Dev Mode"
#define GENERAL_OPTIONS_DEV_MODE_X 18
#define GENERAL_OPTIONS_DEV_MODE_Y 461
#define GENERAL_OPTIONS_DEV_MODE_W 210
#define GENERAL_OPTIONS_DEV_MODE_H 22
#define GENERAL_OPTIONS_DEV_MODE_EVENTID 1016
#define GENERAL_OPTIONS_DEBUG_XYZ_TEXT "Show Debug XYZ"
#define GENERAL_OPTIONS_DEBUG_XYZ_X 18
#define GENERAL_OPTIONS_DEBUG_XYZ_Y 494
#define GENERAL_OPTIONS_DEBUG_XYZ_W 210
#define GENERAL_OPTIONS_DEBUG_XYZ_H 22
#define GENERAL_OPTIONS_DEBUG_XYZ_EVENTID 1017
	...
	switch(event)
	{
		...
		case GENERAL_OPTIONS_OK_EVENTID:
		{
			...
			if(pWindow && pWindow->getInternalID() == GUI_WINDOW_GENERALOPTIONS)
			{
				...
				pCheckBox = SDL_static_cast(GUI_CheckBox*, pWindow->getChild(GENERAL_OPTIONS_EXTRA_RIGHT_EVENTID));
				if(pCheckBox)
					g_engine.setExtraRightSidebar(pCheckBox->isChecked());
				pCheckBox = SDL_static_cast(GUI_CheckBox*, pWindow->getChild(GENERAL_OPTIONS_EXTRA_LEFT_EVENTID));
				if(pCheckBox)
					g_engine.setExtraLeftSidebar(pCheckBox->isChecked());
				bool devMode = g_engine.hasDevMode();
				pCheckBox = SDL_static_cast(GUI_CheckBox*, pWindow->getChild(GENERAL_OPTIONS_DEV_MODE_EVENTID));
				if(pCheckBox)
				{
					devMode = pCheckBox->isChecked();
					g_engine.setDevMode(pCheckBox->isChecked());
				}
				pCheckBox = SDL_static_cast(GUI_CheckBox*, pWindow->getChild(GENERAL_OPTIONS_DEBUG_XYZ_EVENTID));
				if(pCheckBox)
					g_engine.setShowDebugCoordinates(devMode && pCheckBox->isChecked());
...
void UTIL_generalOptions()
{
	...
	newCheckBox->startEvents();
	newWindow->addChild(newCheckBox);
	newCheckBox = new GUI_CheckBox(iRect(GENERAL_OPTIONS_EXTRA_RIGHT_X, GENERAL_OPTIONS_EXTRA_RIGHT_Y, GENERAL_OPTIONS_EXTRA_RIGHT_W, GENERAL_OPTIONS_EXTRA_RIGHT_H), GENERAL_OPTIONS_EXTRA_RIGHT_TEXT, g_engine.hasExtraRightSidebar(), GENERAL_OPTIONS_EXTRA_RIGHT_EVENTID);
	newCheckBox->setBoxEventCallback(&general_options_Events, GENERAL_OPTIONS_EXTRA_RIGHT_EVENTID);
	newCheckBox->startEvents();
	newWindow->addChild(newCheckBox);
	newCheckBox = new GUI_CheckBox(iRect(GENERAL_OPTIONS_EXTRA_LEFT_X, GENERAL_OPTIONS_EXTRA_LEFT_Y, GENERAL_OPTIONS_EXTRA_LEFT_W, GENERAL_OPTIONS_EXTRA_LEFT_H), GENERAL_OPTIONS_EXTRA_LEFT_TEXT, g_engine.hasExtraLeftSidebar(), GENERAL_OPTIONS_EXTRA_LEFT_EVENTID);
	newCheckBox->setBoxEventCallback(&general_options_Events, GENERAL_OPTIONS_EXTRA_LEFT_EVENTID);
	newCheckBox->startEvents();
	newWindow->addChild(newCheckBox);
	...
	newCheckBox->startEvents();
	newWindow->addChild(newCheckBox);
	// Dev Mode / Debug XYZ checkboxes are intentionally omitted from the public
	// client so the developer tools (UI Inspector, Recorder, Low Profile) cannot
	// be enabled by end users. The dev build keeps them.

...
static void loginDebugLog(const char* msg)
{
	g_logger.addLog(LOG_CATEGORY_INFO, std::string(msg));
	FILE* f = nullptr;
	fopen_s(&f, "debug.log", "a");
	if(f) { fprintf(f, "[TCP] %s\n", msg); fclose(f); }
}
...
void ProtocolLogin::parseMessage(InputMessage& msg)
{
	...
	SDL_snprintf(g_buffer, sizeof(g_buffer), "TCP login recv opcode: 0x%02X", SDL_static_cast(Uint32, header));
	loginDebugLog(g_buffer);
...
{
	SDL_snprintf(g_buffer, sizeof(g_buffer), "TCP login connected - protocol=%u client=%u dat=%u spr=%u pic=%u",
		SDL_static_cast(Uint32, Protocol::getProtocolVersion()),
		SDL_static_cast(Uint32, Protocol::getClientVersion()),
		g_datRevision, g_sprRevision, g_picRevision);
	loginDebugLog(g_buffer);
...
{
	loginDebugLog(m_lastError != CONNECTION_ERROR_INVALID ? "TCP login disconnected with error" : "TCP login disconnected (no error)");
	if(m_lastError != CONNECTION_ERROR_INVALID)
	{
		switch(m_lastError)
		{
			case CONNECTION_ERROR_RESOLVE_HOST:
				loginDebugLog("Error: Cannot resolve host");
				UTIL_messageBox("Connection Failed", "Cannot connect to a login server.\n\nError: Cannot resolve host.");
				break;
			case CONNECTION_ERROR_CANNOT_CONNECT:
				loginDebugLog("Error: Cannot connect");
				UTIL_messageBox("Connection Failed", "Cannot connect to a login server.\n\nError: Cannot connect.");
				break;
			case CONNECTION_ERROR_FAIL_CONNECT:
				loginDebugLog("Error: Connection failed");
				UTIL_messageBox("Connection Failed", "Cannot connect to a login server.\n\nError: Connection failed.");
				break;
			case CONNECTION_ERROR_REFUSED_CONNECT:
				loginDebugLog("Error: Connection refused");
				UTIL_messageBox("Connection Failed", "Cannot connect to a login server.\n\nError: Connection refused.");
				break;
			case CONNECTION_ERROR_TIMEOUT:
				loginDebugLog("Error: Connection timed out");
				UTIL_messageBox("Connection Failed", "Cannot connect to a login server.\n\nError: Connection timed out.");
				break;
			case CONNECTION_ERROR_RECV_FAIL:
				loginDebugLog("Error: Failed to call recv()");

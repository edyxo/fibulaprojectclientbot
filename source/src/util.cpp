...
void UTIL_MessageBox(bool fatalError, const char* message)
{
	if(fatalError)//If you want UTF-8 support you must convert your message(if you haven't already)
		SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Tibia-Eddie - Fatal Error", message, NULL);
	else
		SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Tibia-Eddie - Error", message, NULL);
}
...
void UTIL_OpenURL(const char* url)
{
	...
	#else
	SDL_snprintf(g_buffer, sizeof(g_buffer), "xdg-open %s", url);
	system(g_buffer);
...
void UTIL_SafeEventHandler(void (*eHandler)(Uint32, Sint32), Uint32 param, Sint32 status)
{
	SDL_Event event;
	event.type = SDL_USEREVENT;
	event.user.code = CLIENT_EVENT_SAFEEVENTHANDLER;
	event.user.windowID = SDL_static_cast(Uint32, status);
	event.user.data1 = reinterpret_cast<void*>(eHandler);
...
void UTIL_FastCopy_SSE41(Uint8* dst, const Uint8* src, size_t size)
{
	...
	if((SDL_reinterpret_cast(size_t, src) & 15) == 0)
	{
		for(size_t i = size / 64; i--;)
		{
			values[0] = _mm_stream_load_si128((__m128i*)(src + 0));
			values[1] = _mm_stream_load_si128((__m128i*)(src + 16));
			values[2] = _mm_stream_load_si128((__m128i*)(src + 32));
			values[3] = _mm_stream_load_si128((__m128i*)(src + 48));

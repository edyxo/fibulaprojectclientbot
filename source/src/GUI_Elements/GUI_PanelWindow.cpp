...
bool GUI_PanelWindow::isResizeHandleAt(Sint32 x, Sint32 y)
{
	if(!m_windowed || m_tRect.y2 <= 19)
		return false;

	iRect resizeStrip = iRect(m_tRect.x1, m_tRect.y1 + m_tRect.y2 - 4, m_tRect.x2, 4);
	return resizeStrip.isPointInside(x, y);
}

GUI_Element* GUI_PanelWindow::getElementAt(Sint32 x, Sint32 y)
{
	for(std::vector<GUI_Element*>::reverse_iterator it = m_childs.rbegin(), end = m_childs.rend(); it != end; ++it)
	{
		if((*it)->getRect().isPointInside(x, y))
		{
			GUI_Element* element = (*it)->getElementAt(x, y);
			return (element ? element : (*it));
		}
	}
	return NULL;
}

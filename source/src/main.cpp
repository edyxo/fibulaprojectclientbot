...
int main(int argc, char* argv[])
{
	...
	// Read/write the automap (.map minimap tiles) next to the executable so the
	// minimap shipped with the client loads on first run (plug-and-play). The old
	// behaviour stored the automap under the per-user prefs folder (%APPDATA%),
	// which ignored any .map files placed beside the .exe. Fall back to the prefs
	// folder only if the base path is unavailable.
	if(!g_basePath.empty())
		g_mapPath = g_basePath;
	else
	{
	...
	while(g_running)
	{
		...
		while(g_inited)
		{
			...
			{
				SDL_SetWindowTitle(g_engine.m_window, PRODUCT_NAME);

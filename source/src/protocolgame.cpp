...
void ProtocolGame::parseDistanceEffect(InputMessage& msg)
{
	...
	g_map.checkDistanceEffects();
	g_engine.handleComboLeaderDistanceEffect(fromPos, toPos, type);
...
{
	extern Uint16 g_lastLookId;
	extern Position g_lastLookPos;
	g_lastLookId = thingId;
	g_lastLookPos = position;
	OutputMessage msg(getHeaderPos());

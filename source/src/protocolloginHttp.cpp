...
static void debugWriteLog(const char* msg)
{
	FILE* f = nullptr;
...
void handleRequest(Uint32, Sint32 internalId)
{
	if(g_protocolLoginHttp.getRequestId() == SDL_static_cast(Uint32, internalId))
	{
		...
		{
			debugWriteLog("[HTTP] Request succeeded - parsing response");
			// Log the raw response from login.dat
			SDL_snprintf(g_buffer, sizeof(g_buffer), "%slogin.dat", g_prefPath.c_str());
			FILE* f2 = nullptr;
			fopen_s(&f2, g_buffer, "rb");
			if(f2)
			{
				fseek(f2, 0, SEEK_END); long sz = ftell(f2); fseek(f2, 0, SEEK_SET);
				std::vector<char> raw(sz + 1, 0);
				fread(raw.data(), 1, sz, f2); fclose(f2);
				raw[sz] = 0;
				debugWriteLog("[HTTP] Response:");
				debugWriteLog(raw.data());
			}
			g_protocolLoginHttp.parseMessage();
		}
		else
		{
			debugWriteLog("[HTTP] Request FAILED");
...
{
	// Clear previous log
	FILE* clf = nullptr; fopen_s(&clf, "debug.log", "w"); if(clf) fclose(clf);
	...
	std::string jsonStr = JSON_VALUE::encode(json.get());

	#if CLIENT_OVVERIDE_VERSION == 0
	std::string host = g_engine.getClientHost();
	#else
	std::string host = CLIENT_OVERRIDE_LOGIN_HOST;
	#endif

	{
		char logbuf[512];
		SDL_snprintf(logbuf, sizeof(logbuf), "[HTTP] POST to: %s", host.c_str());
		debugWriteLog(logbuf);
		SDL_snprintf(logbuf, sizeof(logbuf), "[HTTP] Payload: %s", jsonStr.c_str());
		debugWriteLog(logbuf);
	}

	m_requestId = g_http.addRequest(host, g_buffer, jsonStr, &handleRequest, 0);
}

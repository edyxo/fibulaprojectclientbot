...
Uint16 g_lastLookId = 0;
Position g_lastLookPos(0xFFFF, 0xFFFF, 0xFF);
bool g_lastLookDebug = false;
...
{
	g_engine.captureAlertEvent("Death");
...
void Game::processContainerOpen(Uint8 containerId, ItemUI* item, const std::string& name, Uint8 capacity, bool hasParent, bool canUseDepotSearch, bool isUnlocked, bool hasPages, Uint16 containerSize, Uint16 firstIndex, std::vector<ItemUI*>& itemVector)
{
	...
	m_containers[containerId]->setOpenOrder(m_nextContainerOpenOrder++);
...
void Game::processTextMessage(MessageMode mode, const std::string& text, Uint32 channelId)
{
	switch(mode)
	{
		...
		case MessageLook:
		{
			...
			bool showDebugLook = (g_engine.hasDevMode() && g_lastLookDebug);
			bool showDebugCoordinates = (showDebugLook && g_engine.hasShowDebugCoordinates() && g_lastLookPos.x != 0xFFFF);
			if(g_lastLookId != 0)
			{
				if(showDebugLook)
				{
					...
					if(showDebugCoordinates)
					{
						len = SDL_snprintf(g_buffer, sizeof(g_buffer), "\n[ID: %u, XYZ: %u,%u,%u]",
							SDL_static_cast(Uint32, g_lastLookId), SDL_static_cast(Uint32, g_lastLookPos.x),
							SDL_static_cast(Uint32, g_lastLookPos.y), SDL_static_cast(Uint32, g_lastLookPos.z));
					}
					else
						len = SDL_snprintf(g_buffer, sizeof(g_buffer), "\n[ID: %u]", SDL_static_cast(Uint32, g_lastLookId));
					displayText += std::string(g_buffer, SDL_static_cast(size_t, len));
				}
				g_lastLookId = 0;
			}
			else if(showDebugCoordinates)
			{
				Sint32 len = SDL_snprintf(g_buffer, sizeof(g_buffer), "\n[XYZ: %u,%u,%u]",
					SDL_static_cast(Uint32, g_lastLookPos.x), SDL_static_cast(Uint32, g_lastLookPos.y),
					SDL_static_cast(Uint32, g_lastLookPos.z));
				displayText += std::string(g_buffer, SDL_static_cast(size_t, len));
			}
			if(g_lastLookPos.x != 0xFFFF)
			{
				g_lastLookPos = Position(0xFFFF, 0xFFFF, 0xFF);
			}
			g_lastLookDebug = false;
			g_map.addOnscreenText(ONSCREEN_MESSAGE_CENTER_HIGH, mode, displayText);
			if(g_engine.hasShowInfoMessages())
				g_chat.addChannelMessage(CHANNEL_ID_SERVERLOG, mode, 0, "", 0, displayText, time(NULL));

			// Parse exiva response: "X is [far/very far] to the DIRECTION." or "X is in the vicinity."
			{
				auto vpos = text.find(" is in the vicinity");
				if(vpos != std::string::npos)
				{
					g_engine.setExiva(8, text.substr(0, vpos));
				}
				else
				{
					static const char* s_prefixes[] = {" is to the ", " is far to the ", " is very far to the ", nullptr};
					for(int pi = 0; s_prefixes[pi]; ++pi)
					{
						auto dpos = text.find(s_prefixes[pi]);
						if(dpos != std::string::npos)
						{
							std::string rest = text.substr(dpos + SDL_static_cast(size_t, SDL_strlen(s_prefixes[pi])));
							Sint8 dir = -1;
							if(rest.size() >= 10 && rest.substr(0, 10) == "north-east") dir = 1;
							else if(rest.size() >= 10 && rest.substr(0, 10) == "north-west") dir = 7;
							else if(rest.size() >= 10 && rest.substr(0, 10) == "south-east") dir = 3;
							else if(rest.size() >= 10 && rest.substr(0, 10) == "south-west") dir = 5;
							else if(rest.size() >= 5 && rest.substr(0, 5) == "north") dir = 0;
							else if(rest.size() >= 5 && rest.substr(0, 5) == "south") dir = 4;
							else if(rest.size() >= 4 && rest.substr(0, 4) == "east") dir = 2;
							else if(rest.size() >= 4 && rest.substr(0, 4) == "west") dir = 6;
							if(dir >= 0)
								g_engine.setExiva(dir, text.substr(0, dpos));
							break;
						}
					}
				}
	...
	switch(mode)
	{
		...
		case MessageSpell:
		{
			if(UTIL_onWhiteList(name) || !UTIL_onBlackList(name))
			{
				...
				{
					// Trigger exhaust bar only when the say text contains a complete
					// Tibia spell word. Splits text into words and checks against the
					// set of all magic root words used in Tibia 7.72 spells.
					// Complete Tibia 7.72 spell word set.
					// A say text is a spell if any space-split word exactly matches one of these.
					static const std::unordered_set<std::string> spellWords = {
						// instant spells — healing
						"exura","exeta","exana","exani","exiva","adura",
						// instant spells — attack
						"exori",
						// instant spells — support / movement
						"utani","utevo","utamo","uthal","utana","utlan",
						// instant spells — fields / area
						"exevo",
						// rune words (first magic word of each rune spell)
						"adori","adana","adevo","adeta","adito","adana",
						// misc
						"alana","aleta"
					};
					bool isSpell = false;
					...
					if(isSpell)
					{
						auto& knownCreatures = g_map.getKnownCreatures();
						for(auto& kv : knownCreatures)
						{
							Creature* c = kv.second;
							if(c && c->getCurrentPosition() == position)
							{
								g_engine.recordExhaust(c->getId());
								break;
							}
						}
					}
		...
		case MessagePrivateFrom:
		{
			if(UTIL_onWhiteList(name) || (!g_engine.getIgnorePrivateMessages() && !UTIL_onBlackList(name)))
			{
				...
				if(g_engine.getAutoLogoutOnPM())
					g_engine.doAutoLogout(("PM from: " + name).c_str());
			}
			else
				g_chat.ignoreListEvent();
		}
		...
		case MessageGamemasterBroadcast:
		{
			...
			if(g_engine.getAutoLogoutOnBroadcast())
				g_engine.doAutoLogout("Broadcast message received");
		}
		break;
		case MessageGamemasterPrivateFrom:
		{
			...
			if(privateChannel)
				...
				g_chat.addChannelMessage(CHANNEL_ID_SERVERLOG, mode, statementId, name, playerLevel, text, time(NULL));
			if(g_engine.getAutoLogoutOnBroadcast())
				g_engine.doAutoLogout(("GM message from: " + name).c_str());
		}
		break;
		default: break;
	}
...
{
	// Match the classic client feel: when chase is enabled, a new attack should
	// interrupt map-click autowalk and let combat movement take priority.
	if(creature && g_engine.getChaseMode() == CHASEMODE_FOLLOW)
		stopAutoWalk();
...
void Game::sendLookAt(const Position& position, Uint16 itemId, Uint8 stackpos, bool debugLook)
{
	if(g_engine.isIngame())
	{
		ProtocolGame* game = GET_SAFE_PROTOCOLGAME;
...
void Game::sendBugReport(const std::string& comment)
{
	if(g_engine.isIngame() && m_canReportBugs)
	{
		ProtocolGame* game = GET_SAFE_PROTOCOLGAME;
		if(game && game->canPerformAction())
			game->sendBugReport(comment);
	}
}
...
void Game::checkMovement(Direction dir, bool queueIfBusy)
{
	if(m_autoWalkDestination.x != 0xFFFF)
		stopAutoWalk();
	...
	Creature* player = g_map.getLocalCreature();
	if(!queueIfBusy && player && (player->isPreWalking() || player->isWalking()))
	{
		m_playerCurrentDir = DIRECTION_INVALID;
		m_playerMovement = DIRECTION_INVALID;
		return;

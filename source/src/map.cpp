...
void Map::renderInformations(Sint32 px, Sint32 py, Sint32 pw, Sint32 ph, float scale, Sint32 scaledSize)
{
	...
	// Exiva direction indicator
	if(g_engine.isExivaActive())
	{
		Sint8 exDir = g_engine.getExivaDir();
		const std::string& exName = g_engine.getExivaName();
		auto& renderer = g_engine.getRender();

		// Compass widget: 64x64, bottom-center of game area
		const Sint32 BOX_W = 64;
		const Sint32 BOX_H = 64;
		Sint32 boxX = px + (pw / 2) - (BOX_W / 2);
		Sint32 boxY = py + ph - BOX_H - 6;
		Sint32 cx = boxX + BOX_W / 2;
		Sint32 cy = boxY + BOX_H / 2;

		// Background + border
		renderer->fillRectangle(boxX, boxY, BOX_W, BOX_H, 0, 0, 0, 170);
		renderer->drawRectangle(boxX, boxY, BOX_W, BOX_H, 1, 90, 90, 90, 200);

		// Direction dot offsets from center (radius 22)
		static const Sint32 s_dx[9] = {0, 16, 22, 16, 0, -16, -22, -16, 0};
		static const Sint32 s_dy[9] = {-22, -16, 0, 16, 22, 16, 0, -16, 0};

		// Draw line from center to active direction
		if(exDir >= 0 && exDir < 8)
		{
			Sint32 tx = cx + s_dx[exDir];
			Sint32 ty = cy + s_dy[exDir];
			if(s_dx[exDir] == 0)
			{
				// vertical (N or S)
				Sint32 lineY = UTIL_min<Sint32>(cy, ty);
				Sint32 lineH = SDL_abs(ty - cy);
				renderer->fillRectangle(cx - 1, lineY, 2, lineH, 255, 255, 255, 100);
			}
			else if(s_dy[exDir] == 0)
			{
				// horizontal (E or W)
				Sint32 lineX = UTIL_min<Sint32>(cx, tx);
				Sint32 lineW = SDL_abs(tx - cx);
				renderer->fillRectangle(lineX, cy - 1, lineW, 2, 255, 255, 255, 100);
			}
			else
			{
				// diagonal: L-shape
				Sint32 midX = tx;
				renderer->fillRectangle(UTIL_min<Sint32>(cx, midX), cy - 1, SDL_abs(midX - cx), 2, 255, 255, 255, 100);
				renderer->fillRectangle(midX - 1, UTIL_min<Sint32>(cy, ty), 2, SDL_abs(ty - cy), 255, 255, 255, 100);
			}
		}

		// Draw 8 direction dots + active highlight
		for(int d = 0; d < 8; ++d)
		{
			bool active = (d == SDL_static_cast(int, exDir));
			Sint32 dotX = cx + s_dx[d] - (active ? 3 : 2);
			Sint32 dotY = cy + s_dy[d] - (active ? 3 : 2);
			Sint32 dotS = active ? 6 : 4;
			renderer->fillRectangle(dotX, dotY, dotS, dotS,
				active ? 255 : 55, active ? 220 : 55, active ? 0 : 55, active ? 255 : 180);
		}

		// Vicinity: large center cross
		if(exDir == 8)
		{
			renderer->fillRectangle(cx - 3, cy - 3, 6, 6, 255, 220, 0, 255);
			renderer->fillRectangle(cx - 6, cy - 2, 12, 4, 255, 220, 0, 160);
			renderer->fillRectangle(cx - 2, cy - 6, 4, 12, 255, 220, 0, 160);
		}
		else
		{
			// Center white dot
			renderer->fillRectangle(cx - 2, cy - 2, 4, 4, 255, 255, 255, 200);
		}

		// Creature name above the indicator box
		if(!exName.empty())
			g_engine.drawFont(CLIENT_FONT_OUTLINED, cx, boxY - 14, exName, 255, 220, 0, CLIENT_FONT_ALIGN_CENTER);
	}
